...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 font-size-18">Cuisine Category</h4>
        </div>
    </div>
</div>
<!-- end page title -->
    ...
    <div class="col-md-4">
        <div class="card">
            <div class="card-body">
                ...
                {!! Form::open([
                'method' => 'POST',
                'action' => 'AdminCuisineCategoryController@store',
                'files' => true,
                'class' => 'form-horizontal',
                'name' => 'addcuisinecategoryForm',
                ]) !!}
                @csrf
                ...
                <div class="row">
                    <div class="mb-3">
                        ...
                        <input type="text" name="category_name" class="form-control text-uppercase" id="category_name" placeholder="Enter name" value="{{ old('category_name') }}" required>
                ...
                <div class="row">
                    <div class="mb-3">
                        <label for="file">Backgroung Image<span class="text-danger">*</span></label>
        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex align-items-center mb-3">
                    <h4 class="card-title mb-0">Cuisine Category List</h4>
                </div>
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap w-100 mt-3">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <a href="{{ route('admin.cuisine_category.edit', $category->id) }}"
                                    class="btn btn-outline-primary waves-effect waves-light" data-bs-toggle="tooltip" data-bs-placement="top"
                                    title="Edit"><i class="fa fa-edit"></i></a>
                                <a href="{{ route('admin.cuisine_category.destroy', $category->id) }}"
                            ...
                            <td>
                                <div class="form-check form-switch">
                                    <input class="form-check-input toggle-status-switch" type="checkbox"
                                        id="toggleSwitch{{ $category->id }}" data-id="{{ $category->id }}"
                                        {{ $category->is_active ? 'checked' : '' }}>
                                    <label class="form-check-label" for="toggleSwitch{{ $category->id }}">
                                        {{ $category->is_active ? 'Show' : 'Hide' }}
                                    </label>
                                </div>
                            </td>
                        </tr>
...
<script>
    $(function() {
        $("form[name='addcuisinecategoryForm']").validate({
            rules: {
                category_name: {
                    required: true,
                },
...
<script>
    $(document).on('change', '.toggle-status-switch', function() {
        ...
        $.ajax({
            url: "{{ route('admin.cuisine_category.active') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                id: agentId
