...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 font-size-18">Dish</h4>
        </div>
    </div>
</div>
<!-- end page title -->
    ...
    <div class="col-md-4">
        <div class="card">
            <div class="card-body">
                ...
                {!! Form::open([
                'method' => 'POST',
                'action' => 'AdminDishesController@store',
                'files' => true,
                ...
                {{-- <div class="row">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" id="name"
                            placeholder="Enter Designation" value="{{ old('name') }}" required>
                @if ($errors->has('name'))
        ...
        <div class="card-body">
            ...
            <table id="datatable" class="table table-bordered dt-responsive nowrap w-100 mt-3">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <a href="{{ route('admin.dishes.edit', $dishe->id) }}"
                                class="btn btn-outline-primary waves-effect waves-light" data-bs-toggle="tooltip" data-bs-placement="top"
                                title="Edit"><i class="fa fa-edit"></i></a>
                            <a href="{{ route('admin.dishes.destroy', $dishe->id) }}"
                                onclick="return confirm('Sure ! You want to delete ?');"
                                class="btn btn-outline-danger waves-effect waves-light" data-bs-toggle="tooltip" data-bs-placement="top"
                                title="Delete"><i class="fa fa-trash"></i></a>
                        </td>
                        <td>
                            <img src="{{ $dishe->file }}" alt="{{ $dishe->file }}" width="50px" hight="50px" loading="lazy">
                        </td>
                        <td>
                            <div class="form-check form-switch">
                                <input class="form-check-input toggle-status-switch" type="checkbox"
                                    id="toggleSwitch{{ $dishe->id }}" data-id="{{ $dishe->id }}"
                                    {{ $dishe->is_active ? 'checked' : '' }}>
                                <label class="form-check-label" for="toggleSwitch{{ $dishe->id }}">
                                    {{ $dishe->is_active ? 'Show' : 'Hide' }}
                                </label>
                            </div>
                        </td>
                    </tr>
...
<script>
    $(function() {
        $("form[name='addDishForm']").validate({
            rules: {
                file: {
                    required: true,
                },
