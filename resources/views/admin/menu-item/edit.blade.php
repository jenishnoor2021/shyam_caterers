...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 font-size-18">Edit Menu Item</h4>
        </div>
    </div>
</div>
<!-- end page title -->
    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body">
                ...
                {!! Form::model($item, [
                'method' => 'PATCH',
                'action' => ['AdminMenuItemController@update', $item->id],
                'files' => true,
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="mb-3">
                            <label for="categories_id">Menu Categpory<span class="text-danger">*</span></label>
                            <select name="categories_id" id="category" class="form-control" style="width:100%" required>
                                <option value="">Select category</option>
                                @foreach($category as $cate)
                                <option value="{{$cate->id}}" {{ $item->categories_id == $cate->id ? 'selected' : '' }}>{{$cate->category_name}}</option>
                                @endforeach
                            </select>
...
<script>
    $(function() {
        ...
        $("form[name='edititemform']").validate({
            rules: {
                categories_id: {
                    required: true,
                },
