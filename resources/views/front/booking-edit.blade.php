      ...
      <div class="card-body">
        ...
        <form action="{{ route('bookings.update', $booking->id) }}" id="editBookingForm" method="POST">
          ...
          <div class="table-responsive d-none d-md-block">
            <table class="table table-bordered align-middle" id="functionTable">
              ...
              <tbody id="functionRows">
                ...
                <tr>
                  <td>
                    <select name="functions[{{ $index }}][fun_id]" class="form-select function-select" required>
                      <option value="">-- Select Function --</option>
                      @foreach ($functionOptions as $option)
                      <option value="{{ $option->id }}" {{ $function['fun_id'] == $option->id ? 'selected' : '' }} data-time="{{ $option->time }}">
          ...
          <div class="d-block d-md-none" id="functionCardContainer">
            @foreach($functions as $index => $function)
            <div class="function-card mb-3" data-index="{{ $index }}">
                ...
                <div class="card-body">
                  <div class="row">
                    ...
                    <div class="col-12 mb-3">
                      ...
                      <input type="number" name="functions[{{ $index }}][person]" class="form-control" value="{{ $function['person'] }}" placeholder="Enter person" required>
                    </div>
                    <div class="col-12 mb-3">
                      <label class="form-label fw-bold">Date &amp; Time <span class="text-danger">*</span></label>
                      <input type="text" name="functions[{{ $index }}][datetime]" class="form-control datetimepicker" placeholder="Enter date time" value="{{ \Carbon\Carbon::parse($function['datetime'])->format('Y-m-d H:i') }}" required>
                    </div>
                    <div class="col-12">
                      <div class="action-mobile text-end">
                        @if(count($functions) > 1)
                        <button type="button" class="btn btn-sm btn-danger" onclick="removeFunctionCard(this)"><i class="fa fa-trash"></i> Remove</button>
                        @endif
                      </div>
                    </div>
                  </div>
